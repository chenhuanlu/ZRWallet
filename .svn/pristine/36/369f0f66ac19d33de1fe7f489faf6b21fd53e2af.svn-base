//
//  RemberPayPasswordViewController.m
//  ZRwalletForConsumer
//
//  Created by 陈焕鲁 on 15/7/26.
...
@interface RemberPayPasswordViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
{
    NSArray *keys;
}
...
@implementation RemberPayPasswordViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    keys = @[@{@"title":@"原始密码",@"key":@"oldPayPwd",@"alert":@"请输入原始密码"},
             @{@"title":@"新密码",@"key":@"newPayPwd",@"alert":@"请输入新密码"},
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.textField.text = self.infoDict[dict[@"key"]];
    self.textField.placeholder = [NSString stringWithFormat:@"请输入%@",dict[@"title"]];
    //********************************************footview********************************
...
- (BOOL)checkPageInput
{
    ...
    if(![self.infoDict[@"newPayPwd"] isEqualToString:self.infoDict[@"confirmpassword"]])
    {
...
- (void)changePwd
{
    NSDictionary *dict = @{@"oldPayPwd":[SecurityTool md5:self.infoDict[@"oldPayPwd"]],
                           @"newPayPwd":[SecurityTool md5:self.infoDict[@"newPayPwd"]]};
    [[HttpRequest sharedRequest] sendRequestWithMessage:nil path:@"changePayPassword" param:dict succuss:^(id result) {
        
        [SVProgressHUD showSuccessWithStatus:@"支付密码更改成功"];
