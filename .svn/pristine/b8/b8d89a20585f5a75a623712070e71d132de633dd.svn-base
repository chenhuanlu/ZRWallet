...
-(void)setup {
    ...
    percentLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 20, self.bounds.size.width, 25)];
    [percentLabel setFont:[UIFont fontWithName:@"Futura-CondensedMedium" size:15]];
 ...
 *  是否显示小圆形进度
 *
 *  @param showLittleCicle <#showLittleCicle description#>
 */
- (void)setShowLittleCicle:(BOOL)showLittleCicle
{
    if (showLittleCicle)
    {
        percentLabel.hidden = YES;
    }
    percentLayer.showCicle = showLittleCicle;
    [percentLayer setNeedsDisplay];
}
...
- (void)setCustomText:(NSString *)string{
    ...
    if ([customText length] == 0) {
        [percentLabel setFont:[UIFont fontWithName:@"Futura-CondensedMedium" size:18]];
