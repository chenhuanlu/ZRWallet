...
- (void)drawXY
{
    ...
    UIView *xView = [[UIView alloc]initWithFrame:CGRectMake(10, self.viewForBaselineLayout.frame.size.height-labelXaxisOffset-10, self.viewForBaselineLayout.frame.size.width-20, 0.5)];
    xView.backgroundColor = [UIColor colorWithRed:215/255.0 green:215/255.0 blue:14/255.0 alpha:1];
    [self addSubview:xView];
    
    
    UIView *YView = [[UIView alloc]initWithFrame:CGRectMake(10+self.distance, 0, 0.5, self.viewForBaselineLayout.frame.size.height-labelXaxisOffset-10)];
    YView.backgroundColor = [UIColor colorWithRed:215/255.0 green:215/255.0 blue:14/255.0 alpha:1];
    [self addSubview:YView];
    
//    [labelXAxis setCenter:CGPointMake(width*(i*numberOfGaps - 1)+10+30, self.frame.size.height - labelXaxisOffset)];
    
    float distance = (self.viewForBaselineLayout.frame.size.height-labelXaxisOffset-10-10)/3;
    for (int i=0; i<3; i++)
    {
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(10, 10+i*distance, self.distance, 20)];
        label.font = [UIFont systemFontOfSize:10];
        label.textAlignment = NSTextAlignmentCenter;
        label.adjustsFontSizeToFitWidth = YES;
        label.backgroundColor = [UIColor clearColor];
        label.textColor = [UIColor colorWithRed:215/255.0 green:215/255.0 blue:14/255.0 alpha:1];
        label.text = [NSString stringWithFormat:@"%.0f-",[self maxValue]/3*(3-i)];
        [self addSubview:label];
        
        UIView *lineView = [[UIView alloc]init];
        lineView.frame = CGRectMake(10+self.distance, label.center.y, self.viewForBaselineLayout.frame.size.width-20-self.distance,  0.5);
        lineView.backgroundColor = [UIColor colorWithRed:215/255.0 green:215/255.0 blue:14/255.0 alpha:1];
...
//        float width =(self.viewForBaselineLayout.frame.size.width-10-self.distance-20)/(numberOfPoints-1);
        float width =(self.viewForBaselineLayout.frame.size.width-10-self.distance-20)/([self.delegate numberOfDatasInGraph]-1);
        positionOnXAxis = width*i+10+self.distance;
        positionOnYAxis = (self.viewForBaselineLayout.frame.size.height - 50) - ((dotValue - minValue) / ((maxValue - minValue) / (self.viewForBaselineLayout.frame.size.height -50))) + 20;
...
//    int numberOfGaps = [self.delegate numberOfGapsBetweenLabels] + 1;
    int numberOfGaps = [self.delegate numberOfGapsBetweenLabels] ;
//    float width =(self.viewForBaselineLayout.frame.size.width-10-self.distance-20)/(numberOfPoints-1);
    float width =(self.viewForBaselineLayout.frame.size.width-10-self.distance-20)/([self.delegate numberOfDatasInGraph]-1);
    ...
    if (numberOfGaps >= (numberOfPoints - 1)) {
        ...
        int max;
        int numOfData = [self.delegate numberOfDatasInGraph];
        if (numOfData%numberOfGaps==0)
        {
          max = numOfData/numberOfGaps-1;
        }
        else
        {
           max = numOfData/numberOfGaps;
        }
...
//            labelXAxis.text = [self.delegate labelOnXAxisForIndex:(i * numberOfGaps - 1)];
            ...
            labelXAxis.text = [self.delegate labelOnXAxisForIndex:(i * numberOfGaps)];
...
//            [labelXAxis setCenter:CGPointMake((self.viewForBaselineLayout.frame.size.width/(numberOfPoints-1))*(i*numberOfGaps - 1), self.frame.size.height - labelXaxisOffset)];
              ...
              [labelXAxis setCenter:CGPointMake(width*i*numberOfGaps +10+self.distance, self.frame.size.height - labelXaxisOffset)];
            
            labelXAxis.font = self.labelFont;
            labelXAxis.textAlignment = NSTextAlignmentCenter;
//            labelXAxis.textColor = self.colorXaxisLabel;
            labelXAxis.textColor = [UIColor colorWithRed:215/255.0 green:215/255.0 blue:14/255.0 alpha:1];
            labelXAxis.backgroundColor = [UIColor clearColor];
            [self addSubview:labelXAxis];
            
            
//            UIView *lineView = [[UIView alloc]init];
//            lineView.frame = CGRectMake(labelXAxis.center.x, 0, 0.5,  self.viewForBaselineLayout.frame.size.height-labelXaxisOffset-10);
//            lineView.backgroundColor = [UIColor colorWithRed:215/255.0 green:215/255.0 blue:14/255.0 alpha:1];;
//            [self addSubview:lineView];
        }
    }
